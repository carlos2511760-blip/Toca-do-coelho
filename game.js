...
function gameLoop(ts) { if (gameState !== 'PLAYING') return; let dt = (ts - lastTime) / 1000; if (dt > 0.1) dt = 0.1; lastTime = ts; gameTime += dt; gameTimerEl.innerText = formatTime(gameTime); player.update(dt); currentRoom.update(dt); enemies.forEach(e => e.update(dt)); projectiles.forEach(p => p.update(dt)); particles = particles.filter(p => p.update(dt)); icebergs = icebergs.filter(ib => ib.update(dt)); checkCollisions(); updateCooldowns(); ctx.clearRect(0, 0, 800, 600); ctx.save(); if (screenShakeT > 0) { ctx.translate((Math.random() - .5) * screenShakeM, (Math.random() - .5) * screenShakeM); screenShakeT -= dt; } currentRoom.drawBase(ctx); enemies.forEach(e => e.draw(ctx)); projectiles.forEach(p => p.draw(ctx)); particles.forEach(p => p.draw(ctx)); player.draw(ctx); ctx.restore(); lightCtx.globalCompositeOperation = 'source-over'; lightCtx.fillStyle = 'rgba(10, 10, 15, 0.85)'; lightCtx.fillRect(0, 0, 800, 600); lightCtx.globalCompositeOperation = 'destination-out'; let drawLight = (X, Y, R, int) => { let g = lightCtx.createRadialGradient(X, Y, 0, X, Y, R); g.addColorStop(0, `rgba(0,0,0,${int})`); g.addColorStop(1, 'rgba(0,0,0,0)'); lightCtx.fillStyle = g; lightCtx.beginPath(); lightCtx.arc(X, Y, R, 0, Math.PI * 2); lightCtx.fill(); }; drawLight(player.x, player.y, 250, 1); projectiles.forEach(p => drawLight(p.x, p.y, p.radius * 8, 0.7)); particles.forEach(p => drawLight(p.x, p.y, p.size * 6, 0.5)); enemies.forEach(e => { if (['boss', 'miniboss'].includes(e.type)) drawLight(e.x, e.y, e.radius * 4, 0.5); }); icebergs.forEach(i => drawLight(i.x, i.y, 80, 0.4)); pickups.forEach(p => drawLight(p.x, p.y, 80, 0.5)); if (['shop', 'treasure', 'spawn', 'npc'].includes(currentRoom.type)) drawLight(400, 300, 450, 0.6); lightCtx.globalCompositeOperation = 'source-over'; ctx.drawImage(lightCanvas, 0, 0); currentRoom.drawUI(ctx); requestAnimationFrame(gameLoop); }
